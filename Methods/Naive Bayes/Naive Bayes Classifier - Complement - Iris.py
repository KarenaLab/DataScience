...
for Sample in Sample_List:
    ...
    print(f" > Performing Complement Naive Bayes for Sample = {int(Sample)}%")

    Sample = Sample/100
    X_Train, X_Test, Y_Train, Y_Test = train_test_split(Data_X, Data_Y, test_size= Sample, random_state= 27)

    Complement_NaiveBayes = ComplementNB()
    Complement_NaiveBayes.fit(X_Train, Y_Train)

    Y_Pred_Train = Complement_NaiveBayes.predict(X_Train)
    Y_Pred_Test = Complement_NaiveBayes.predict(X_Test)
...
Title = "Complement Naive Bayes Classifier - Iris Petals"
